...
**esison.dev** is a personal portfolio and blog website for Eric Sison, built with Astro 6 as a static site with Vue 3 interactive islands, deployed on Netlify.
...
- `src/pages/index.astro` - Entry point: homepage with hero, skills, featured projects, GitHub project sync section, work history
- `src/components/GitHubProjectsSection.astro` - Build-time GitHub-backed repository cards for the homepage
- `src/lib/github-projects.ts` - GitHub API fetch, validation, normalization, and fallback logic for homepage repos
- `src/consts.ts` - Global constants (`SITE_TITLE`, `SITE_DESCRIPTION`) and GitHub project sync settings
...
- `.github/workflows/refresh-github-projects.yml` - Scheduled workflow that triggers Netlify rebuilds for GitHub project refreshes
...
- **HTTP/JSON contract:** `POST /.netlify/functions/reveal_contact` with JSON body (`token`, `honeypot`, `tNow`, `includePhone`, `phoneToken` — see `reveal_contact.js` for field meaning). Responses are JSON.
  - **`200`:** Primary path: `{ email, phone }` — `phone` is a string when revealed, otherwise `null`. If the user asked for phone but secondary Turnstile failed, email is still returned with `phone: null` and optional `meta: { phoneWithheld: true, reason: "secondary-verification-failed" }`. Phone-only follow-up (`includePhone` + `phoneToken` with empty primary `token`): `{ phone }` only — email is never returned without a verified primary `token`.
  ...
  - **`429`:** `{ error: "rate-limited" }` — client IP exceeded 10 requests in a 15-minute window. Back off and retry after the window expires. In-memory per-container store; resets on cold start.
...
| `ENABLE_GITHUB_PROJECTS` | Enable or disable the homepage GitHub repos section | `'true'` \| `'false'`, optional (server-only, defaults to enabled when unset) |
...
- GitHub Actions secrets — Scheduled Netlify refresh workflow secret (`NETLIFY_BUILD_HOOK_URL`)
...
### GitHub REST API
- **What:** Build-time source of public repository metadata for the homepage GitHub projects section
- **Loaded via:** `src/lib/github-projects.ts` during Astro page generation
- **Key env vars:** `ENABLE_GITHUB_PROJECTS`, `GITHUB_TOKEN`
- **Gotchas:** Repositories only appear when they are public and tagged with `portfolio`; cards display last push time using `pushed_at`; builds fall back to a visible error state if GitHub is unreachable

### Netlify Build Hook
- **What:** Scheduled rebuild trigger so GitHub repo changes propagate to the static site without a manual deploy
...
5. **GitHub project sync is build-time only** — Repository visibility changes appear on the homepage only after the next successful site rebuild.
...
1. Add the `portfolio` topic to a public GitHub repository to show it in the homepage GitHub projects section
2. Remove the `portfolio` topic from that repository to hide it from the homepage
3. Set `ENABLE_GITHUB_PROJECTS=false` to hide the entire GitHub projects section without changing the template
4. Set `GITHUB_TOKEN` if build-time GitHub requests hit rate limits
5. Configure the `NETLIFY_BUILD_HOOK_URL` GitHub Actions secret so scheduled refreshes can trigger Netlify rebuilds automatically
...
| `src/consts.ts` | Global constants (`SITE_TITLE`, `SITE_DESCRIPTION`) and GitHub sync settings |
...
| `.github/workflows/refresh-github-projects.yml` | Scheduled and manual workflow that triggers Netlify rebuilds for GitHub project refreshes |
...
- **GitHub projects are unavailable during build** — Configure `GITHUB_TOKEN` for higher API limits, verify outbound network access, and trigger another build after connectivity is restored.
- **Scheduled GitHub refresh is not running** — Add the `NETLIFY_BUILD_HOOK_URL` GitHub Actions secret so `.github/workflows/refresh-github-projects.yml` can trigger Netlify builds.
